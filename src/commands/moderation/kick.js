const { SlashCommandBuilder, EmbedBuilder, PermissionFlagBits, Embed } = require('discord.js')

module.exports = {
    data: new SlashCommandBuilder()
        .setName('kick')
        .setDescription('Kick a user from the discord server')
        .addUserOption(option => (option.setName('target').setDescription('User to be kicked').setRequired(true)))
    ...
    async execute(interaction, client) {
        ...
        if (userToKick.roles.highest.position >= interaction.member.roles.highest.position)
        ...
        const embed = new EmbedBuilder()
            .setDescription(`Succesfully kicked **${target}** with the reason: **${reason}**`)
